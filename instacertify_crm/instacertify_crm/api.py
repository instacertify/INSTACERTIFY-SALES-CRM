	...
	return [
		{
			...
			"payable_to": getattr(row, "payable_to", None),
			"counts_as_revenue": flt(getattr(row, "counts_as_revenue", 0)),
			"no_of_samples": getattr(row, "no_of_samples", None),
			"accreditation": getattr(row, "accreditation", None),
			"standard_code": getattr(row, "standard_code", None),
			"testing_timeline": getattr(row, "testing_timeline", None),
		}
		for row in rows or []
	]
...
def _notify_quote_owners(quote, title: str, message: str):
	"""Notify the person who shared/owns the quote + IC Admins."""
	users = set()
	if quote.owner:
...
@frappe.whitelist()
def my_dashboard():
	"""Personal greeting strip: pending work related to the signed-in user."""
	user = frappe.session.user
	filters_assigned = {"assigned_to": user}
	my_leads = frappe.db.count(
		"IC Lead",
		{**filters_assigned, "status": ["not in", ["WON", "LOST"]]},
	) if frappe.db.exists("DocType", "IC Lead") else 0
	my_open_quotes = frappe.db.count(
		"IC Quote",
		{"owner": user, "status": ["in", ["Draft", "Shared", "Revision Requested"]]},
	) if frappe.db.exists("DocType", "IC Quote") else 0
	my_projects = frappe.db.count(
		"IC Customer Project",
		{**filters_assigned, "status": ["not in", ["Closed", "Lost", "Delivered"]]},
	) if frappe.db.exists("DocType", "IC Customer Project") else 0
	my_pending_samples = (
		frappe.db.count(
			"IC Sample Request",
			{"status": ["not in", ["Shared with Customer", "Closed"]], "owner": user},
		)
		if frappe.db.exists("DocType", "IC Sample Request")
		else 0
	)
	hours_today = 0
	if frappe.db.exists("DocType", "IC Working Hour Log"):
		hours_today = (
			frappe.db.sql(
				"""
				SELECT COALESCE(SUM(hours), 0) FROM `tabIC Working Hour Log`
				WHERE `user`=%s AND work_date = CURDATE()
				""",
				(user,),
			)[0][0]
			or 0
	...
	return {
		"my_leads": my_leads,
		"my_open_quotes": my_open_quotes,
		"my_projects": my_projects,
		"my_pending_samples": my_pending_samples,
