...
IC_ROLES = (
	...
	"IC Sales Person",
	"IC Sales Ops",  # legacy alias kept for existing sites
)
...
def role_labels():
	"""Human labels for Desk onboarding."""
	return {
		"IC Admin": "Oversees all data; Excel export everywhere applicable",
		"IC All Ops Manager": "Views everything; authorises operations users below",
		...
		"IC Sales Ops": "Legacy sales/ops role (prefer IC Sales Person / IC Operations Manager)",
	}
...
def _seed_consultants():
	if not frappe.db.exists("DocType", "IC Consultant"):
		return
	if not frappe.db.exists("IC Consultant", "External Referral Desk"):
		frappe.get_doc(
			{
				"doctype": "IC Consultant",
				"consultant_name": "External Referral Desk",
				"company": "Partner Consultants",
				"email": "partners@instacertify.in",
				"phone": "+91 9999118039",
				"active": 1,
				"notes": "Default library entry — replace with real consultant names.",
			}
		).insert(ignore_permissions=True)


def _seed_holidays():
	if not frappe.db.exists("DocType", "IC Holiday"):
		return
	# Seed Republic Day / Independence Day placeholders for current year if empty
	if frappe.db.count("IC Holiday"):
		return
	year = frappe.utils.now_datetime().year
	for name, month, day, htype in (
