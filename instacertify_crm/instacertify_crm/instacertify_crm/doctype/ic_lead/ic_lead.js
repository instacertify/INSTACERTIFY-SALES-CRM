...
frappe.ui.form.on("IC Lead", {
	refresh(frm) {
		...
		if (!frm.is_new()) {
			const quoteType =
				frm.doc.request_type === "Testing Request" ? "Testing" : "Service";
			frm
				.add_custom_button(__("Create Quote"), () => {
					frappe.new_doc("IC Quote", {
						...
						subject:
							frm.doc.request_type === "Certificate Renewal"
								? "Certificate Renewal"
								: "",
					});
				})
	...
	lead_source(frm) {
		frm.toggle_display("consultant", frm.doc.lead_source === "Consultant");
	},
});

frappe.listview_settings["IC Lead"] = {
	add_fields: ["assigned_to", "status", "follow_up_on", "lead_cost", "request_type"],
...
function toggle_state(frm) {
	...
	frm.set_df_property(
		"state",
		"description",
		india ? "Indian state (required)" : "Optional — leave blank if outside India",
	);
}
